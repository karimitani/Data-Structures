...
MyString::MyString(const char * s) {initialize(s);}

// Create a MyString from a copy of another string
MyString::MyString(const MyString &s) {initialize(s._s);}
...
void MyString::remove(int i, int n)
{
    ...
    {
    	int j;
    	for (j = i; j < length() - 1; j++) { _s[j] = _s[j + 1];}
    }  
    _s[length() - n] = '\0';
...
bool MyString::operator == (const MyString & s)
{
    int i;
    i = 0;  
    while ((_s[i] != '\0' && s._s[i] != '\0') && (_s[i] == s._s[i])) {i++;}
...
const char * MyString::cStr() {return _s;}

// Get string length of this string.
int MyString::length() const {return slength(_s);}

// Destructor. Deallocate the space used by _s
MyString::~MyString() {delete [] _s;}
