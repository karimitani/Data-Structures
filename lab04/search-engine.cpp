...
SearchEngine::SearchEngine( int port, DictionaryType dictionaryType):MiniHTTPD(port){
	...
	int i;
	for (i = 0; i < 1024; i++) {records[i] = new URLRecord();}
...
void SearchEngine::dispatch( FILE * fout, const char * documentRequested) {
	...
	for (i = 0; i < listCount; i++) {
		...
		for (j = 0; j < numWords; j++) {
			...
			int exists = 0;
			while (data != NULL) {
				if (data->_urlRecord == list[i]){exists = 1;}
				data = data->_next;
			}	
			if (exists == 0)
				list[i] = NULL;
...
int main(int argc, char ** argv) {
	...
	if (!strcmp(dictType, "array")) {dictionaryType = ArrayDictionaryType;}
	else if (!strcmp(dictType, "avl")){dictionaryType = AVLDictionaryType;}
	else if (!strcmp(dictType, "bsearch")) {dictionaryType = BinarySearchDictionaryType;}
	else
	{
		printUsage();
		return 0;
