...
// Constructor
BinarySearchDictionary::BinarySearchDictionary() {sorted = false;}
...
DataType BinarySearchDictionary::findRecord(KeyType key) {
	...
	while (max >= min) {
		...
		if (result == 0){return (DataType)array[mid].data;}
...
void BinarySearchDictionary::sort() {
	...
	for (i = currentNumber / 2; i >= 0; i--){siftDown(i, currentNumber - 1);}
...
void BinarySearchDictionary::siftDown(int root, int bottom) {
	int complete;
	int max;
	ArrayDictionaryNode temp;
	complete = 0;
	while ((root * 2) <= bottom && complete == 0) {
		...
		else if (strcmp(array[(root * 2)].key, array[(root * 2) + 1].key) > 0) {max = (root * 2);}
		else {max = (root * 2) + 1;}
