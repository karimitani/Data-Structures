...
MiniHTTPD::MiniHTTPD( int port ) {_port = port;}
...
void MiniHTTPD::run() {
  ...
  // Assign port 
  if (_port > 0) {serverAddress.sin_port = htons((u_short) _port);}
  else {
  ...
  if (gethostname(hostName, MAXHOSTNAMELEN) != 0) {strcpy(hostName, "localhost");}
...
void MiniHTTPD::dispatch( FILE * fout, const char * documentRequested ) {
