...
public class XmlDataSource {
    ...
    public String getBitmapFileName() {return _bitmapFile;}
    
    public void setBitmapFileName(String value) {_bitmapFile = value;}
    
    public double getFeetPerPixel() {return _scale;}
    
    public void setFeetPerPixel(double value) {_scale = value;}
    ...
    public Location getLocationByPoint(int x, int y, int radius) {
        for (Location l : Locations) {
            if (((x - radius) <= l.getX() && l.getX() <= (x + radius)) && ((y - radius) <= l.getY() && l.getY() <= (y + radius))) {return l;}
        }
        return null;
    }
    ...
    public void writeFile(String fileName){
        try{
            ...
            for (Location l : Locations) {pw.println("<location id=\"" + l.getID() + "\" name=\"" + l.getName() + "\" x=\"" + l.getX() + "\" y=\"" + l.getY() + "\" />");}
            for (Path p : Paths) {pw.println("<path idfrom=\"" + p.getFrom() + "\" idto=\"" + p.getTo() + "\" />");}
            pw.println("</mapfile>");
            pw.close();
        }
        catch (Exception ex) {ex.printStackTrace(System.out);}
