...
public class MapScene implements Scene
{
    ...
    public void changeNotify() {if (_listener != null) _listener.stateChanged(null);}
    ...
    public void mouseClicked(Point pt) {
        if (_parent.CURRENT_MODE == 1) { }
            ...
            if (l != null) {
                Path target = null;
                for (Path p : _parent._data.Paths) {
                    if (p.getFrom() == l.getID() || p.getTo() == l.getID()) {
                        target = p;
                    }
                }
                if (target != null) {
            ...
            for (Path p : _parent._data.Paths) {
                ...
                if (line.ptSegDist((double)pt.x, (double)pt.y) <= 8.0){ target = p;}
            }
